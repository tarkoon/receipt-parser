...
COMMON_FIELDS = {
    "document_type", "merchant", "date", "time", "location", "currency",
...
def validate_file(path: Path) -> tuple[list[str], list[str]]:
    ...
    if time_val is not None and not TIME_RE.match(str(time_val)):
        errors.append(f'time "{time_val}" does not match HH:MM format')
...
def _validate_receipt(data: dict, errors: list, warnings: list):
    ...
    # sum(line_items) ≈ subtotal (外税) or ≈ total (内税)
    if items:
        items_sum = sum(it.get("total", 0) for it in items if isinstance(it, dict))
        total_val = data.get("total")
        sub_ok = isinstance(subtotal, (int, float)) and abs(items_sum - subtotal) <= 2
        tot_ok = isinstance(total_val, (int, float)) and abs(items_sum - total_val) <= 2
        if (subtotal is not None or total_val is not None) and not (sub_ok or tot_ok):
            errors.append(
                f"sum of line item totals ({items_sum}) matches neither "
                f"subtotal ({subtotal}) nor total ({total_val})"
            )
